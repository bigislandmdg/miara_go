...
export function DriverHome({
  ...
  try {
    ...
    // 🔥 Supprime localement sans re-fetch
   ...
   const deleteRideRequest = async (id: string) => {
    ...
    } catch { Alert.alert("Erreur", "Impossible de supprimer la demande."); }
  };
  ...
  const filteredTrips = useMemo(() => {
    return trips.filter(t => {
        if (
      filterDate &&
      normalizeDate(t.date) !== filterDate
    )
      return false
      if (
        filterDeparture &&
        !t.departure.toLowerCase().includes(filterDeparture.toLowerCase())
      )
        return false
      if (
        filterDestination &&
        !t.arrival.toLowerCase().includes(filterDestination.toLowerCase())
      )
        return false
      if (
        filterVehicle &&
  ...
  return rideRequests.filter(r => {
    ...
    if (
      filterDate &&
      normalizeDate(r.desired_date) !== filterDate
    )
      return false;
  ...
  }, [rideRequests, filterDate, filterDeparture, filterDestination]);
  ...
  <TouchableOpacity
    ...
    onPress={() =>
      Alert.alert(
        ...
        type === "trip"
          ? "Supprimer ce trajet ?"
          : "Supprimer cette demande de trajet ?",
        [
...
const renderTrip = ({ item }: { item: DriverTrip }) => {
          ...
          {item.vehicle && (
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailText}>
                ...
                {item.vehicle.availableSeats}/
                {item.vehicle.totalSeats} sièges)
              </Text>
            </View>
          )}
        </View>
  ...
  const config =
    requestStatusConfig[item.status ?? "active"];
  ...
  const handlePress = () => {
    ...
    // 🔵 ouvrir PublishScreen
    onViewChange?.("publish");

    // raha mila mandefa data
    // afaka ampidirina ao amin'ny global state
  };
    ...
    <View style={styles.container}>
      ...
      />
        ...
        <View style={styles.statsRow}>
          ...
          <TouchableOpacity style={styles.statCard} activeOpacity={0.85} onPress={() => Alert.alert(`Crédits : ${totalCredits}`)}>
          ...
          : (
            <>
              ...
              {paginatedTrips.length < filteredTrips.length && (
                <TouchableOpacity style={styles.loadMore} onPress={() => setTripPage(p => p + 1)}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                onPress={() => {
                  if (modalType === "departure") setFilterDeparture(modalInput);
                  if (modalType === "destination") setFilterDestination(modalInput);
                  if (modalType === "vehicle") setFilterVehicle(modalInput);

                  if (modalType === "offerPrice") setFilterOfferPrice(modalInput);
                  if (modalType === "offerSeats") setFilterOfferSeats(modalInput);
                  if (modalType === "offerMessage") setFilterOfferMessage(modalInput);
          ...
          : (
            <>
             ...
             {paginatedRideRequests.length < filteredRideRequests.length &&  (
                <TouchableOpacity style={styles.loadMore} onPress={() => setRequestPage(p => p + 1)}>
                  <Text style={styles.loadMoreText}>{t("seeMore")}</Text>
                </TouchableOpacity>
              )}
            </>
...
const styles = StyleSheet.create({
  ...
  filterChip: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 14, backgroundColor: "#E5E7EB" },
